...
PERIOD = ['26 April 2021','28 April 2021']
RISK_REWARDS = [[2,1],[1.5,1],[3,2],[1.3,1],[1.8,1],[3,2],[4,1],[3,1],[4,2]]
MARKETS = ['manausdt','xrpusdt']
TIMEFRAMES = ['5m','15m']
...
def multiple_backtest(market_arr,timeframe_arr,risk_arr,range_start,range_end):
    ...
    for kls in KLINES:
        ...
        o = np.array(kls[count][:,1],dtype=np.float64)
        h = np.array(kls[count][:,2],dtype=np.float64)
        l = np.array(kls[count][:,3],dtype=np.float64)
        c = np.array(kls[count][:,4],dtype=np.float64)

        EMA50 = talib.WMA(c,50)
        RSI = talib.RSI(c,14)
        ATR = talib.ATR(h,l,c)
        computed_methods = compute_methods(o,h,l,c)
        TEST_ALL(computed_methods,d,o,h,l,c,EMA50,RSI,ATR,risk_arr,market_arr[count],timeframe_arr[count])
    ...
    while not completed:
        if count < len(o)-1:
            ...
            if total:
                ...
                gains = ( (len(PROFIT) * riskreward[0]) - len(LOSS) * riskreward[1] )
...
multiple_backtest(MARKETS,TIMEFRAME,RISK_REWARDS,PERIOD[0],PERIOD[1])