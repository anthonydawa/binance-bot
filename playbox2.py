        ...
        if len(bls) == 3:
            if x200 < z:
                ...
                if candle_oldest < 0 and candle_prev < 0 and candle_now > 0 and abs(candle_prev) < candle_now :
                    ...
                    if abs(candle_prev) <= 0.02 and candle_now <= 0.1 :
                    ...
                    elif candle_now > 0.1 and xrsi < 55 and xrsi > 30:
                        if not sl:
                        ...
                        #     with open('engulfing-trades', 'a') as fs:
                        #         fs.write(f'{dayx},{x50},{x200},{xatr},{xrsi},{z},{zx},{tp}\n')
                        elif not sl and xrsi > 55:
                            print('not buying candle rsi is overbought', xrsi)



                    elif xrsi < 30 or xrsi > 60:
...
#######################################################################
        ...
        if len(bls) == 3:
            if not rsi_counter:
                ...
                if xrsi > 35 and xrsi < 55 and bls[2] > 0 and bls[1] > 0 and z > x200:
                    tp = z + (1.9) * (xatr)
                    slp = z - (1) * (xatr)
                    buy_market_order(mrkt,sl,slp,loss_streak,dec=deci)
                    sell_limit_order(tp,symbol,mrkt,order,sl,price_limit_dec,dec=deci)
        ...
        if not ct:
            ...
            if len(rt) == 4:
                ...
                if z > x50:
                    ...
                    with open('engulfing-trades', 'a') as fs:
                        fs.write(f'{dayx},{x50},{x200},{xatr},{xrsi},{z},{zx}\n')
                    print('enter signal!')
                    print('stoploss:',sls)
                    print('profit:',pts)
                    rt.clear()
...
def buy_market_order(mrkt,sl,sl_price,loss_streak,amt_percent=0.999,us='USDT',min_trade=30,dec=4):

    if loss_streak:
        print('LOSS STREAK POPPED!')
        while loss_streak:
            loss_streak.pop()

    elif not loss_streak:
        ...
        quantity = (100/price)*(amt_percent)
...
def sell_limit_order(prft,s,mrkt,ordr,sl,price_limit_dec,amt_percent=0.999,min_trade=20,dec=1):
...
def sell_market_order(s,mrkt,stpls,loss_streak,amt_percent=0.999,min_trade=20,dec=4):
    balance = client.get_asset_balance(asset=s)
    balance_amt = float(balance['free'])
    trades = client.get_recent_trades(symbol=mrkt)
    trade_value = float(trades[0]['price'])
